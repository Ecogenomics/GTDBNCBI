...
def add_tax_to_arbfile(type_info_file, arb_file):
    
    map_genus_to_info = {}
    map_species_to_info = {}
    
    info_fh = open(type_info_file, "rb")
    for line in info_fh:
    ...
    for line in arb_fh:
        ...
        elif (splitline[0] == "warning") and (current_id is not None) and (species_name is not None):
            information = []
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(prog='add_typestrain_to_arb_file.py')
    parser.add_argument('-t', dest='type_info_file', required=True,
                        help='File containing type info')
    parser.add_argument('-a', dest='arbfile', required=True, 
                        help='Arb txt file')
    args = parser.parse_args()
    
    add_tax_to_arbfile(args.type_info_file, args.arbfile) 