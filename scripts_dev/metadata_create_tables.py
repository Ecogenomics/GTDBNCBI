                ...
                for species_dir in os.listdir(domain_dir):
                    ...
                    for assembly_dir in os.listdir(full_species_dir):
                        ...
                        protein_file = os.path.join(
                            full_assembly_dir, "prodigal", genome_id + "_protein.faa")

                        metadata_nt_file = os.path.join(
                            full_assembly_dir, self.metadata_nt_file)
                        self._parse_nt(accession, metadata_nt_file, fout_nt)

                        metadata_gene_file = os.path.join(
                            full_assembly_dir, self.metadata_gene_file)
                        self._parse_gene(
                        ...
                        ssu_count = self._parse_taxonomy_file(
                            accession, ssu_silva_taxonomy_file, fout_ssu_silva_taxonomy, 'ssu_silva', ssu_silva_fna_file, ssu_silva_summary_file)
        ...
        if user_genome_dir != 'NONE':
            for user_id in os.listdir(user_genome_dir):
                ...
                for genome_id in os.listdir(full_user_dir):
                    ...
                    metadata_nt_file = os.path.join(
                        full_genome_dir, self.metadata_nt_file)
                    self._parse_nt(genome_id, metadata_nt_file, fout_nt)

                    metadata_gene_file = os.path.join(
                        full_genome_dir, self.metadata_gene_file)
                    self._parse_gene(genome_id, metadata_gene_file, fout_gene)

                    ssu_gg_taxonomy_file = os.path.join(
                        full_genome_dir, self.ssu_gg_taxonomy_file)
                    ssu_gg_fna_file = os.path.join(
                        full_genome_dir, self.ssu_gg_fna_file)
                    self._parse_taxonomy_file(
                        genome_id, ssu_gg_taxonomy_file, fout_gg_taxonomy, 'ssu_gg', ssu_gg_fna_file)

                    ssu_silva_taxonomy_file = os.path.join(
                        full_genome_dir, self.ssu_silva_taxonomy_file)
                    ssu_silva_fna_file = os.path.join(
                        full_genome_dir, self.ssu_silva_fna_file)
                    ssu_silva_summary_file = os.path.join(
                        full_genome_dir, self.ssu_silva_summary_file)
                    ssu_count = self._parse_taxonomy_file(
                        genome_id, ssu_silva_taxonomy_file, fout_ssu_silva_taxonomy, 'ssu_silva', ssu_silva_fna_file, ssu_silva_summary_file)

                    lsu_silva_23s_taxonomy_file = os.path.join(
                        full_genome_dir, self.lsu_silva_23s_taxonomy_file)
                    lsu_silva_23s_fna_file = os.path.join(
                        full_genome_dir, self.lsu_silva_23s_fna_file)
                    lsu_silva_23s_summary_file = os.path.join(
                        full_genome_dir, self.lsu_silva_23s_summary_file)
                    lsu_23s_count = self._parse_taxonomy_file(
                        genome_id, lsu_silva_23s_taxonomy_file, fout_lsu_silva_23s_taxonomy, 'lsu_silva_23s', lsu_silva_23s_fna_file, lsu_silva_23s_summary_file)

                    lsu_5S_fna_file = os.path.join(
                        full_genome_dir, self.lsu_5S_fna_file)
                    lsu_5S_summary_file = os.path.join(
                        full_genome_dir, self.lsu_5S_summary_file)
                    lsu_5S_count = self._parse_lsu_5S_files(
                        genome_id, fout_lsu_5S, lsu_5S_fna_file, lsu_5S_summary_file)
                    ...
                    trna_file = os.path.join(
                        full_genome_dir, 'trna', genome_id + '_trna_stats.tsv')
                    self._parse_trna_file(
                        genome_id, trna_file, fout_trna_count)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'genbank_genome_dir', help='base directory leading to NCBI GenBank archaeal and bacterial genome assemblies')
    parser.add_argument(
        'refseq_genome_dir', help='base directory leading to NCBI RefSeq archaeal and bacterial genome assemblies')
    parser.add_argument(
