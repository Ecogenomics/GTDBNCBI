...
class InfoGenerator(object):
    ...
    def __init__(self, cpus, output_dir):
        ...
        logger_setup(args.output_dir, "gtdb_type_information_table.log", "gtdb_type_information_table", __version__, False)
    ...
    def parse_ncbi_names_and_nodes(self, ncbi_names_file, ncbi_nodes_file, taxids_of_interest):
        ...
        self.logger.info('Identified %d NCBI taxonomy species nodes.' % len(species_taxids))
        ...
        self.logger.info('Associated %d NCBI taxon nodes with their parent species node.' % len(species_of_taxid))
            ...
            for line in nnf:
                ...
                if tokens[3] == 'type material':
                    for sid in self._fix_common_strain_id_errors([tokens[1]]):
                        type_material[cur_taxid].add(self._standardize_strain_id(sid))
        ...
        self.logger.info('Read auxillary species name information for %d NCBI taxIDs.' % len(category_names))
        self.logger.info('Read type material information for %d NCBI taxIDs.' % len(type_material))
    ...
    def load_metadata(self, metadata_file):
        ...
        with open(metadata_file) as metaf:
            ...
            gtdb_ncbi_type_material_designation_index = headers.index('ncbi_type_material_designation')
            gtdb_accession_index = headers.index('accession')
            gtdb_taxonomy_species_name_index = headers.index('ncbi_taxonomy')
            gtdb_strain_identifiers_index = headers.index('ncbi_strain_identifiers')
            gtdb_ncbi_taxonomy_unfiltered_index = headers.index('ncbi_taxonomy_unfiltered')
            ...
            for line in metaf:
                ...
                if not infos[gtdb_accession_index].startswith('U_'):
                    ...
                    if infos[gtdb_strain_identifiers_index] != 'none':
                        pattern = re.compile('[\W_]+')
                        created_list = [sid.strip() for sid in infos[gtdb_strain_identifiers_index].split(';')]
                        created_list = self._fix_common_strain_id_errors(created_list)
    ...
    def get_species_name(self, gid):
        ...
        ncbi_unfiltered_taxa = [t.strip() for t in self.metadata[gid]['ncbi_taxonomy_unfiltered'].split(';')]
        ncbi_species = None
        ...
        if ncbi_subspecies:
            if 'subsp.' not in ncbi_subspecies:
                self.logger.warning("NCBI subspecies name without 'subsp.' definition: %s" % ncbi_subspecies)
    ...
    def strains_iterate(self, gid, standard_name, repository_strain_ids, raw_names, misspelling_names, synonyms, equivalent_names, isofficial, sourcest):
        ...
        for repository_strain_id in repository_strain_ids.split("="):
            ...
            if repository_strain_id in strain_ids:
                ...
                pattern = re.compile('[\W_]+')
                collapsed_names = {pattern.sub('', a).upper(): a for a in raw_names}
    ...
    def strain_match(self, 
        ...
        for standard_name, raw_names in standard_names.items():
            ...
            if not istype and sourcest == 'lpsn':
                repository_strain_ids = strain_dictionary.get(standard_name).get('neotypes')
            ...
            if istype or isneotype:
                ...
                if not isofficial:
                    heterotypic_synonym = self.check_heterotypic_synonym(standard_name, official_standard_names)
            ...
            m = self.Match(category, istype, isneotype, gtdb_type_status, standard_name, matched_strain_id, year_date)
            if category == 'official_name':
                ...
                # depending on the different synonyms, equivalent names, and misspelling
                if match:
                    prev_gtdb_type_status = match.gtdb_type_status
    ...
    def parse_strains(self, sourcest, strain_dictionary, outfile):
        ...
        try:
            ...
            writeProc = mp.Process(target = self._writer, args = (sourcest, outfile, writer_queue))
    ...
    def _worker(self, 
        ...
        while True:
            ...
            unofficial_standard_names = self.standardise_names(unofficial_potential_names)

            # match species and strain information from NCBI with information at type repository (e.g., LPSN)
            ...
            if match:
                if sourcest == 'lpsn':
                    # lpsn has information for both strains and neotype strains
                    repository_strain_ids = strain_dictionary[match.standard_name].get('strains')
                else:
    ...
    def _writer(self, sourcest, outfile, writer_queue):
        ...
        fout = open(outfile, 'w')
        fout.write('genome\tncbi_organism_name\tncbi_species_name\tncbi_type_designation\tgtdb_type_designation')
        fout.write('\tncbi_base_strain_ids\tncbi_canonical_strain_ids\tmatched_strain_id')
        fout.write('\t{0}_match_type\t{0}_match_name\t{0}_match_strain_id\t{0}_strain_ids'.format(sourcest))
        ...
        while True:
            ...
            if info_genomes['ncbi_taxid'] in self.ncbi_auxiliary_names:
                misspelling = '; '.join(self.ncbi_auxiliary_names[info_genomes['ncbi_taxid']]['misspelling'])
                equivalent_name = '; '.join(self.ncbi_auxiliary_names[info_genomes['ncbi_taxid']]['equivalent name'])
                synonym = '; '.join(self.ncbi_auxiliary_names[info_genomes['ncbi_taxid']]['synonym'])
            ...
            fout.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                    ...
                    '; '.join(self.metadata[gid]['ncbi_expanded_standardized_strain_ids']),
                    matched_strain_id,
                    category_name,
                    matched_sp_name,
                    '; '.join(repository_strain_ids.intersection(self.metadata[gid]['ncbi_expanded_standardized_strain_ids'])),
            ...
            processed += 1
            statusStr = '-> Processing %d genomes assembled from type material.'.ljust(86) % processed
    ...
    def type_summary_table(self,
        ...
        fout.write("accession\tncbi_species\tncbi_organism_name\tncbi_strain_ids\tncbi_canonical_strain_ids")
        fout.write("\tncbi_taxon_authority\tncbi_type_designation")
        fout.write("\tgtdb_type_designation\tgtdb_type_designation_sources")
        fout.write("\tlpsn_type_designation\tdsmz_type_designation\tstraininfo_type_designation")
        fout.write('\tlpsn_priority_year\tdsmz_priority_year\tstraininfo_priority_year')
        ...
        self.logger.info('Genomes where GTDB and NCBI both designate type strain of subspecies: %d' % agreed_type_of_subspecies)
        ...
        for gid, genome_metadata in self.metadata.items():
            ...
            if ncbi_taxid in ncbi_coidentical_strain_ids:
                if strain_ids.intersection(ncbi_coidentical_strain_ids[ncbi_taxid]):
                    ...
                    strain_ids = strain_ids.union(ncbi_coidentical_strain_ids[ncbi_taxid])
            ...
            if ncbi_taxid in ncbi_species_of_taxid:
                ...
                if ncbi_sp_taxid in ncbi_coidentical_strain_ids:
                    if strain_ids.intersection(ncbi_coidentical_strain_ids[ncbi_sp_taxid]):
                        ...
                        # type material strain IDs specified by the NCBI taxonomy
                        # in names.dmp for this taxon
                        strain_ids = strain_ids.union(ncbi_coidentical_strain_ids[ncbi_sp_taxid])
    ...
    def run(self, 
        ...
        self.logger.info('Parsing NCBI taxonomy information from names.dmp and nodes.dmp.')
        rtn = self.parse_ncbi_names_and_nodes(ncbi_names_file, ncbi_nodes_file, taxids_of_interest)
        ...
        # expand set of NCBI co-identical strain IDs associated with each genome
        self.logger.info('Expanding co-indetical strain IDs associated with each genome.')
        self._expand_ncbi_strain_ids(ncbi_coidentical_strain_ids, ncbi_species_of_taxid)
        ...
        if sourcest == 'dsmz' or sourcest == 'all':
            ...
            dsmz_summary_file = os.path.join(self.output_dir, 'dsmz_summary.tsv')
        ...
        if sourcest == 'straininfo' or sourcest == 'all':
            ...
            self.straininfo_strains_dic = self.load_straininfo_strains_dictionary(straininfo_dir)
            
            self.logger.info('Processing StrainInfo data.')
            straininfo_summary_file = os.path.join(self.output_dir, 'straininfo_summary.tsv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--metadata_file', help='Metadata file generated by GTDB')
    parser.add_argument('--ncbi_names', help='NCBI names.dmp file')
    parser.add_argument('--ncbi_nodes', help='NCBI nodes.dmp file')
    parser.add_argument('--cpus', help='Number of threads.', type=int, default=1)
