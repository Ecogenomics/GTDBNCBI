...
class StrainParser(object):
  ...
  def run(self, genome_dir_file,out_dir,header):
      ...
      outfwarning =open(os.path.join(out_dir,header+"_summary_file_warning.txt"),"w")
      outfwarning.write("genome_id\tOrganism name\tstrain (assembly report)\tstrain (genomic gbff)\tstrain (wgsmaster gbff)\ttype_material\n")
      outf.write("genome_id\tOrganism name\tstrain (assembly report)\tstrain (genomic gbff)\tstrain (wgsmaster gbff)\ttype_material\n")
      ...
      with open(genome_dir_file,'r') as genomelistfile :
          for line in genomelistfile:
            ...
            genbank_file = os.path.join(line_split[1], genome_dir_id + '_assembly_report.txt')
                ...
                with open(genbank_file,'r') as gfile:
                    for gline in gfile:
                        if gline.startswith('# Organism name: '):
                            ...
                            #regex = re.compile(".*?\((.*?)\)")
                            species = re.sub(r'\([^)]*\)', '', speline).rstrip().lstrip() 
                        if gline.startswith('# Infraspecific name:'):
                            strline = gline.replace("# Infraspecific name:","")
                            strain = strline.replace("strain=","").rstrip().lstrip()
                        if gline.startswith("# Relation to type material: assembly from type material"):
                            typemat = "True"
                        if typemat =="True" and strain !="" and species != "":
                            break
                            
            wgsmaster_file = os.path.join(line_split[1], genome_dir_id + '_wgsmaster.gbff')
            if os.path.exists(wgsmaster_file) :
                with open(wgsmaster_file,'r') as wfile:
                    for wline in wfile:
                        if pattern.match(wline):
                            wstrain = wline.replace("/strain=",'').replace('"','').rstrip().lstrip()
                                      
            
           
            genomic_file = os.path.join(line_split[1], genome_dir_id + '_genomic.gbff')
            if os.path.exists(genomic_file) :
                with open(genomic_file,'r') as gfile:
                    for gline in gfile:
                        if pattern.match(gline):
                            gstrain = gline.replace("/strain=",'').replace('"','').rstrip().lstrip()
            ...
            if len(set(gstrains)) > 1:
                outfwarning.write("{3}\t{0}\t{1}\t{2}\t{5}\t{4}\n".format(species,strain,gstrains,genome_id,typemat,wstrain))
            else:
                outf.write("{3}\t{0}\t{1}\t{2}\t{5}\t{4}\n".format(species,strain,gstrain,genome_id,typemat,wstrain))
      outf.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--header', help='header that will be concatenated to the output files (gb,rs,gbk,....)')
