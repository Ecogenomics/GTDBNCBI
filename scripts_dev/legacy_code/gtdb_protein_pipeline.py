...
class Pipeline(object):
    ...
    def __init__(self, tmp_dir, output_dir, cpus):
        ...
        check_dependencies(
            ['prodigal', 'hmmsearch', 'pfam_search.pl', 'genometk'])
        ...
        logger_setup(output_dir, "gtdb_protein_pipeline.log",
                     "gtdb_protein_pipeline", __version__, False)
    ...
    def _run_annotation(self, genome_paths, worker_func, db_str):
        ...
        for gid, gpath in list(genome_paths.items()):
            ...
            gene_file = os.path.join(
                prodigal_dir, canonical_gid + self.protein_file_ext)
            if os.path.exists(gene_file):
                if os.stat(gene_file).st_size == 0:
                    self.logger.warning(
                        'Protein file appears to be empty: %s' % gene_file)
                else:
                    gene_files.append(gene_file)
            else:
                self.logger.warning(
                    'Protein file appears to be missing: %s' % gene_file)

        # populate worker queue with data to process
        self.logger.info('Determining %s annotations for %d genomes using %d CPUs.' % (
            db_str, len(gene_files), self.cpus))
        ...
        try:
            workerProc = [mp.Process(target=worker_func, args=(
    ...
    def _protein_metadata(self, genome_paths):
        ...
        for gid, gpath in list(genome_paths.items()):
            ...
            gff_file = os.path.join(
                gpath, 'prodigal', canonical_gid + '_protein.gff')
    ...
    def run(self, genome_list_file, gtdb_genome_path_file):
        ...
        self.logger.info('Identified %d genomes to process.' %
...
if __name__ == '__main__':
    ...
    parser.add_argument('genome_list_file',
                        help='file listing genomes to process')
    parser.add_argument('gtdb_genome_path_file',
                        help='file indicating path to GTDB genomes')
